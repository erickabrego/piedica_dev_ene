...
class WebsiteCalendarController(WebsiteCalendar):
    ...
    def calendar_appointment_submit(self, appointment_type, datetime_str, employee_id, name, phone, email, country_id=False, **kwargs):
        ...
        if appointment_type.p_lead_create:
            if not '/appointment?failed' in res.location:
                ...
                is_paciente = kwargs.get("is_paciente")
                if is_paciente == 'paciente':
                    partner.sudo().write({'x_studio_es_paciente':True})
                elif is_paciente == 'medico':
                    partner.sudo().write({'x_studio_es_mdico_especialista': True})
                    ...
                    if access_token_match:
                        ...
                        if event:
                            ...
                            event.write({
                                ...
                                'x_studio_servicios_1': kwargs.get("x_studio_servicios_1"),
                                'x_send_email_from_website': True,
                                'x_studio_paciente_asisti_a_cita':False,
                                'x_studio_sucursal': appointment_type.x_studio_sucursal_pidica
                            })
                else:
                    if not partner:
    ...
    def calendar_appointment_form(self, appointment_type, employee_id, date_time, **kwargs):
        res = super(WebsiteCalendarController, self).calendar_appointment_form(appointment_type, employee_id, date_time, **kwargs)
        x_studio_servicios_1 = dict(request.env['calendar.event'].sudo()._fields['x_studio_servicios_1'].selection)
        countries = request.env['res.country'].search([('code','in',['MX','US','CO'])])
        res.qcontext.update({"servicios": x_studio_servicios_1,'countries': countries})

        partner_id = request.env['res.partner'].sudo().browse(res.qcontext.get('partner_data').get("id"))
        if partner_id and partner_id.x_studio_es_paciente and not partner_id.x_studio_es_mdico_especialista:
            res.qcontext.update({'paciente':True, 'medico':False})
        elif partner_id and partner_id.x_studio_es_mdico_especialista and not partner_id.x_studio_es_paciente:
            res.qcontext.update({'paciente': False, 'medico':True})
